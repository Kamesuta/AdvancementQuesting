...
test('タスク保存: item 条件を追加して保存するとリロード後も itemType が保持される', async ({ page }) => {
  ...
  await page.mouse.click(box2!.x + box2!.width / 2, box2!.y + box2!.height / 2)
  await expect(page.getByPlaceholder('クエストのタイトル')).toBeVisible({ timeout: 5000 })
  // ItemIcon は CSS background sprite で描画される。title 属性はアトラス座標が見つかった
  // 本物のスプライト div にのみ付与され、ロード中プレースホルダーや色フォールバック SVG には付かない。
  // よって [title*="stone"] が見えた時点でアトラスがビルド済み & stone のスプライトが描画されている。
  // (アトラスのロード待ちで CI ではプレースホルダーのまま数秒かかることがあるため余裕を持たせる)
  const stoneIcon = page.locator('[title*="stone"]').first()
  await expect(stoneIcon).toBeVisible({ timeout: 10000 })

  // toBeVisible だけでは background-image が 404 でも通ってしまうため、
  // アトラス PNG を実際に参照していること & その PNG が 200 で読めることまで確認する。
  const bgImage = await stoneIcon.evaluate(el => getComputedStyle(el).backgroundImage)
  expect(bgImage).toMatch(/\/mc\/atlas\/(blocks|items)\.png/)
  const atlasUrl = bgImage.match(/url\(["']?([^"')]+)["']?\)/)![1]
  const atlasRes = await page.request.get(atlasUrl)
  expect(atlasRes.status()).toBe(200)
  expect(Number(atlasRes.headers()['content-length'] ?? '1')).toBeGreaterThan(0)

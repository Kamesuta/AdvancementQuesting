...
// ---------------------------------------------------------------------------
// add_node モード (スマホ): 配置とノード開きの挙動
// ---------------------------------------------------------------------------

// M-AN-1: 空きスペースタップ → ノード追加のみ、モーダルは開かない
test('スマホ: add_nodeモード — 空きスペースタップでノードが追加されモーダルは開かない (M-AN-1)', async ({ page }) => {
  await loginAs(page, 'demo-editor-token')
  await page.getByTitle('クエストを追加').click()
  ...
  // ノードが存在しない空きスペースをタッチでタップ
  await page.touchscreen.tap(canvasBox!.x + 50, canvasBox!.y + 500)
...
// M-AN-2: 既存ノードタップ → モーダルが開く
test('スマホ: add_nodeモード — 既存ノードタップでモーダルが開く (M-AN-2)', async ({ page }) => {
  await loginAs(page, 'demo-editor-token')
  await page.getByTitle('クエストを追加').click()

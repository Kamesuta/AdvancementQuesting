...
```ts
type ToolMode = 'pan' | 'select' | 'add_node' | 'add_link' | 'delete'
//              ↑新設   ↑クリックでモーダル  既存3つ
```

| ボタン | アイコン | 動作 |
|--------|---------|------|
| 移動 | `Hand` | キャンバスドラッグでパン (ノードクリック不可) |
| 選択 | `MousePointer2` | ノードクリックでモーダル / ドラッグでノード移動 |
| 追加 | `Plus` | キャンバスクリックでクエスト追加 |
| 依存関係 | `ArrowRight` | ノード→ノードで依存線を引く |
| 削除 | `Trash2` | ノードクリックで削除 |
